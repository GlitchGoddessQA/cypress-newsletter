...
const doc = {
  ...
  articleEditBtn(elText: string) {
    return cy
      .get('[class*="inEditMode"]')
      .contains(elText)
      .siblings(":contains(edit)");
  },
  get doneEditBtn() {
...
describe("newsletters", () => {
  beforeEach(() => {
    cy.intercept("POST", "**/api/UserFeedModel/list_newsletter_editions").as(
      "news"
    );
  ...
  it("when edit article name, expected details to be updated", () => {
    doc.articleName
      ...
      .then((artName) => {
        ...
        cy.get('[class*="editLastOptionActions"]')
  ...
  it("when edit article name/description, expected details to be updated on the preview step", () => {
    ...
    doc.articleName
      .invoke("text")
      .then((articleName) => {
        return cy.all<[string, string]>(
          cy.wrap(articleName),
          doc.articleDesc.invoke("text")
        );
      })
  ...
  it("when drag'n'drop element, expected articles to be re-ordered", () => {
    ...
    cy.get(".styles_contentDarkMode__Vmpie")
      ...
      .then(() => {
        cy.get(".styles_handle__EMNKs:eq(0)").drag(
          ".styles_handle__EMNKs:eq(2)",
          {
            force: true,
          }
        );
